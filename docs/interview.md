...
ChainSight is a Java 21 and Spring Boot historical-data warehouse using Ethereum as a public high-volume data source. The implemented backend currently supports Web3j-based block fetching, checkpoint-aware sequential block-range ingestion, JDBC batch inserts, Flyway-managed PostgreSQL schema, and tests for ingestion behavior.
...
- No custom concurrency implementation yet.

Important:

Do not claim custom thread pools, `CompletableFuture`, or concurrent extraction yet.
...
- Transaction receipts are not fetched yet, so transaction status and actual gas used are stored as null for now.
